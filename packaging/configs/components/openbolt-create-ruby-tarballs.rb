...
component "openbolt-create-ruby-tarballs" do |pkg, settings, platform|
  ...
  if platform.is_windows?
    ...
    # Unlike other examples, where the path would be '../<file>' we use the current working directory.
    # This is because even though we add a source as above, because it is not a gem or tar etc.,
    # the component has nothing to extract therefore we don't end up in a component directory.
    ...
    pkg.build do
      ...
      build_commands << (
        "#{platform.tar} --create --gzip --file share/install-tarballs/ruby-#{settings[:ruby_version]}.tgz --directory=. " + dirs.join(" ")
      )
